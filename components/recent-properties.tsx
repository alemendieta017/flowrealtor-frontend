      ...
      <CardContent>
        <div className="space-y-4">
          {properties.map((property) => (
            <div
              key={property.id}
              className="bg-secondary/50 hover:bg-secondary flex items-center gap-4 rounded-lg p-3 transition-colors"
            >
              <img
                src={property.image}
                alt={property.title}
              ...
              <div className="min-w-0 flex-1">
                <h4 className="text-foreground truncate font-medium">{property.title}</h4>
                <div className="mt-1 flex items-center gap-2">
                  <Badge
                    variant={property.type === 'Venta' ? 'default' : 'secondary'}
                    className={
                      property.type === 'Venta' ? 'bg-primary text-primary-foreground' : ''
                    }
                  >
                    {property.type}
                  </Badge>
